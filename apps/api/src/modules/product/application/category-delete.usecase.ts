...
import { CategoryDeleteEntity } from '../domain/category.entity';
...
export class CategoryDeleteUseCase {
  ...
  async execute(id: string, orgId: string) {
    ...
    if (!res) {
      ...
      const data = new CategoryDeleteEntity({
        id,
        businessId,
      });
      return this.categoryService.deleteCategory(data.id, data.businessId);
    }
  }
}
